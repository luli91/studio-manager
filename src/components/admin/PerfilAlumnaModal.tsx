...
export default function PerfilAlumnaModal({ alumna, onClose, onUpdate }: { alumna: any, onClose: () => void, onUpdate: () => void }) {
  ...
  const ejecutarEliminacionAlumna = async () => {
    setEliminando(true)
    try {
  ...
  return (
    <>
          ...
          <div className="p-6 overflow-y-auto space-y-8 flex-1 bg-muted/30">
            ...
            <div className="pt-6 mt-4 border-t border-destructive/20">
              <div className="bg-destructive/10 p-4 rounded-xl border border-destructive/20 flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                <Button variant="destructive" onClick={() => setMostrarConfirmacionBorrado(true)} className="shrink-0 bg-destructive hover:bg-destructive/90 text-destructive-foreground">
      ...
      {/* MODAL DE CONFIRMACIÓN DE BORRADO DE ALUMNA */}
      {mostrarConfirmacionBorrado && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-card rounded-3xl shadow-2xl max-w-sm w-full p-8 text-center space-y-5 border border-destructive">
            ...
            <div>
              <h3 className="font-black text-2xl text-foreground tracking-tighter uppercase italic">¿Borrar Alumna?</h3>
              <p className="text-muted-foreground text-sm mt-2 font-medium leading-relaxed">
                Esta acción no se puede deshacer. Se eliminará la cuenta de <strong>{formData.nombre} {formData.apellido}</strong> y todas sus reservas pendientes.
              </p>
            </div>
            <div className="flex flex-col gap-3 pt-4">
              <Button onClick={ejecutarEliminacionAlumna} disabled={eliminando} variant="destructive" className="w-full rounded-xl font-bold uppercase tracking-wider h-12">
                {eliminando ? <Loader2 className="h-5 w-5 animate-spin mr-2" /> : null}
                Sí, eliminar definitivamente
              </Button>
              <Button onClick={() => setMostrarConfirmacionBorrado(false)} disabled={eliminando} variant="outline" className="w-full rounded-xl font-bold border-border text-foreground hover:bg-accent h-12">
                Cancelar
              </Button>
            </div>
          </div>
